...
- React + TypeScript + Vite，TokenHub 调用由受控 BFF 完成；用户在设置中提供本次会话 Key，服务端不保存、不读取环境变量或 `key.md` 中的 Key。
- 开发服务仅绑定 `127.0.0.1`，并拦截 `key.md`、`.env*`、`.git` 的 HTTP 访问。
- 当前构建产物是静态前端，生产通过 CloudBase HTTP 云函数提供 `/api` BFF；本地 Vite 代理仅用于开发预览。
...
| 生产密钥 | 用户在设置中提供会话 Key，服务端仅请求内存读取并转发 | BFF 不持久化 Key，配合请求限流、审计和 TokenHub 侧轮换 |
