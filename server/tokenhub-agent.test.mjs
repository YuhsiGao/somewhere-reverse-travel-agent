...
const request = (body, init = {}) => new Request('https://example.test/api/agent', { method: 'POST', headers: { 'content-type': 'application/json', ...init.headers }, body, ...init });
...
test('returns a validated profile and does not expose the secret', async () => {
  ...
  assert.match(captured.url, /tokenhub\.test\/v1\/chat\/completions$/); assert.equal(captured.init.headers.authorization, 'Bearer test-secret'); assert.doesNotMatch(JSON.stringify(payload), /test-secret/);
});

test('fails safely for missing configuration and malformed upstream content', async () => {
  const unconfigured = await handler({ env: {} })(request(JSON.stringify({ input: '周末想安静一点' })));
